...
**Опрос MikroTik: бинарный API с постоянной сессией + бэкофф. Бережём журнал роутера.**

Проблема: REST — stateless basic-auth, роутер пишет `logged in via rest-api` на КАЖДЫЙ запрос. При опросе каждые 30с журнал маленького роутера забивается служебными логинами (а место в нём всегда впритык). Отдельно: при недоступном транспорте (напр. сломанный TLS на роутере) контроллер долбил роутер каждые 30с без остановки — реальный кейс на Таурени забил журнал ошибками `ssl: no common ciphers` на 92%.

- **#102 — транспорт `api` (бинарный API, постоянная сессия).** Настройка `mikrotik_transport` (`rest` по умолчанию | `api`). В режиме `api` контроллер логинится ОДИН раз и держит соединение → в журнал роутера попадает один логин, а не тысячи. Порт: **api-ssl 8729 (TLS)** если `mikrotik_https`, иначе **api 8728**. Значения полей строковые — как у REST, потребители (`mt_resource`/`mt_interfaces`/…) не тронуты. `_req` при `api` транслирует путь в `/<path>/print` и маппит singleton (`system/resource`,`system/identity`) в объект, остальное в список. Реконнект при обрыве, сериализация через lock. Записи (если появятся) идут по REST.
- **#101 — бэкофф при серии отказов опроса.** После отказа интервал растёт 60→120→240→300с (потолок) и сбрасывается к штатным 30с на первом успехе. Здоровый роутер не затрагивается. Гейт применяется к обоим транспортам.
- **API настроек MikroTik** (`GET`/`PUT /api/mikrotik/settings`) теперь отдаёт/принимает `https` и `transport`.

Совместимость: дефолт `rest` — поведение существующих контроллеров не меняется, только добавляется бэкофф. Переключение на `api` — по настройке, per-controller (требует включённого api/api-ssl на роутере).

## 3.9.1 — 2026-07-24

**#99 — смена MAC при том же IP больше не плодит запись-призрак.**

Баг: устройство сменило MAC (сброс/замена интерфейса), IP тот же — а идентичность в БД была завязана ТОЛЬКО на MAC. Именованная запись (с name/room/мониторингом) навсегда оставалась `offline`, а полный скан заводил новую БЕЗЫМЯННУЮ запись под новый MAC. Оператор видел на дашборде «нерабочее» устройство, хотя оно пингуется. Найдено на реальном кейсе — точка MikroTik `192.168.95.27` (MAC `…e6:a0` → `…e6:a1`).

- **`merge_scan` детектит смену MAC**: новый MAC ещё не в БД, но по этому IP уже есть **значимая** запись с другим MAC, и старый MAC в этом же скане нигде не виден → усыновляем новый MAC на существующую запись (сохраняя имя/комнату/мониторинг/историю) вместо создания дубля. Запись сразу `online`.
- **Защита от подмены устройства**: ставим флаг `mac_change_pending` + пишем событие `mac_changed` (from→to) → в UI бейдж «⚠ MAC сменился» и баннер в карточке с кнопкой «Подтвердить». `POST /api/network/devices/<id>/ack-mac-change` снимает флаг.
- **Защита от шума**: усыновляем только «значимые» записи (есть name/room, мониторинг, статик-резервация или инфраструктурный тип). Случайные клиенты с рандомизацией MAC (телефоны) не склеиваются. Если старый MAC ещё жив в сети — не усыновляем (замена/2 NIC).
- Схема: `devices.mac_change_pending` + `devices.prev_mac` (PRAGMA-guarded ALTER).

Разовый дубль на Pi3 уже вычищен вручную; фикс предотвращает повторение у любого клиента.

## 3.9.0 — 2026-07-23

**#73 — шифрование секрет-колонок в inventory.db (at-rest).**

Секреты в БД больше не хранятся открытым текстом — при компрометации файла БД (кража бэкапа, доступ к диску) пароли устройств не читаются без отдельного ключа.

- **Fernet (AES-128-CBC + HMAC)** шифрует: `device_credentials.password`, `devices.snmp_community`, и настройки-секреты (`iridium_password`, `mikrotik_password`, `fleet_token`). Значение хранится с префиксом `enc:v1:` — по нему миграция и дешифровка идемпотентны.
- **Ключ — файл `secret.key`** (0600) в `/var/lib/smartcomm-dashboard/`, **отдельно от `inventory.db` и её бэкапов**: бэкап БД (в т.ч. в daily-ротации) без ключа бесполезен. Генерируется при первом старте.
- **Одноразовая миграция** при старте шифрует уже лежащий открытым текстом секрет (idempotent: `enc:v1:`-значения пропускаются). Чтение (`setting_get`/`list_credentials`/snmp) прозрачно дешифрует → остальной код и API отдают plaintext как раньше; MikroTik/iRidium/fleet-token работают без изменений.
- Legacy-совместимость: значение без префикса читается как есть (не ломает БД, зашифрованную частично / до миграции).

Замечание: **старые бэкапы БД, снятые до миграции, содержат plaintext** — вычищаются 30-дневным retention; новые бэкапы уже зашифрованы.

## 3.8.2 — 2026-07-23

**#72 — подпись командного канала (Ed25519), сторона контроллера (fleet_agent).**

Агент теперь **проверяет подпись** каждой команды из heartbeat перед выполнением:
- Публичный Ed25519-ключ портала пинится в настройке `fleet_cmd_pubkey` (ставится при онбординге). Проверяется подпись + `expires_at` (устаревание) + `nonce` (replay — виденные nonce хранятся до их expiry).
- **Ключ настроен → fail-CLOSED**: команда с неверной подписью / просроченная / повторная **отвергается** (не исполняется, репортится `failed: подпись отвергнута`).
- **Ключ НЕ настроен → fail-open** + видимое предупреждение в статусе агента (`cmd_signing`) — обратная совместимость при раскатке. После установки ключа — строгая проверка.

Портальная сторона (подпись) — портал `0.10.4`. Проверено: подпись отвергает подмену любого поля, просрочку и replay.

## 3.8.1 — 2026-07-23

**Остаток #81 (reliability) + #75/#76 (security).**

### Security
- **#76 — web-SSH: пароль устройства больше НЕ уходит в URL** (был виден в history браузера + GET-логах webssh на :8022). В URL только hostname/port/username; пароль кладётся в буфер обмена (fallback `execCommand` для http-контекста), оператор вставляет в поле webssh. index-терминал не затронут (там пароль не подставлялся).
- **#75 — MikroTik REST по HTTPS (www-ssl)** — опция `mikrotik_https` в `_req`: шифрует Basic-auth в проводе (защита пароля роутера от пассивного сниффинга). По умолчанию http (не ломаем контроллеры без www-ssl — оператор включает www-ssl на роутере, затем флипает настройку). Self-signed cert → verify off (цель — шифрование, не cert-auth; пиннинг — отдельный техдолг).

### Reliability (#81, остаток дашборд-репо)
- **#81.4 — PREV_CPU/PREV_NET разделены** между request-path и фоновым sampler'ом. Раньше делили одни глобалы → sampler каждые 60с схлопывал окно расчёта rate для следующего `/api/status` (и наоборот) → шумные/заниженные CPU%/сеть. Теперь у каждого свой store.
- **#81.2 — nmap: жёсткий кап** timeout снижен (per-device `-sV` 90→45с, subnet `-sn` 180→90с), чтобы застрявший скан не держал waitress-поток дольше нужного.

Остаток #81: psycopg2-пул + резолв сессии 1×/запрос — в репо **fleet** (портал), отдельным релизом.

## 3.8.0 — 2026-07-23

**Батч надёжности/корректности из бэклога аудита (#77/#78/#82/#83 + #81 частично).**

### Fixed
- **#83 — метрика uptime% сервисов была неверной.** Брали lifetime `uptime_running_seconds` и клампили к окну через `min()` → любой сервис, проживший дольше 24ч, всегда показывал ~100% (даже если только что упал). Теперь считаем uptime ВНУТРИ окна по `last_status_change_at` (корректно при ≤1 смене статуса в окне).
- **#82 — DB-надёжность (3 части):**
  - Реакция на повреждение БД: `db_integrity_check` при провале сохраняет копию `inventory.CORRUPT-<ts>.db` (иначе daily-backup перезатрёт хорошие копии), пишет CRITICAL и выставляет флаг `db_integrity` в `/api/status` → алерт портала (раньше return игнорировался).
  - **VACUUM раз в 7 дней** (метка в файле, чтобы не гонять на каждый рестарт) — дефрагментация + возврат места после DELETE-ретеншена.
  - **last_seen throttle**: писали на КАЖДЫЙ запрос → churn WAL; теперь не чаще раза в 90с на сессию.
- **#81.1 — `iridium_version()` больше не блокирует.** Блокирующий `sudo IR_BIN --version` (timeout 10с) вынесен в фоновый sampler; функция только читает кеш. Раньше висел на первом `/api/status` и каждые 45с пока irserver недоступен (в т.ч. в heartbeat-потоке).
- **#81.5 — метрик-сэмплер**: stats всех сервисов считаются ДО открытия write-txn (чтение compose.yml — файловый I/O), потом короткая вставка `executemany`. Раньше write-txn держалась открытой на весь цикл чтения.
- **#78 — MikroTik sync**: `ALTER TABLE devices ADD dhcp_static` больше не бросает `OperationalError` на каждый sync — проверяем `PRAGMA table_info` (run-once).

### Removed
- **#77 — мёртвый код** в `services.py`: `get_dependencies` (no-op stub) и `list_internal_containers` (без вызовов).

Осталось из #81: nmap/snmp hard-cap в sync-хендлерах (#81.2, user-triggered), psycopg2-пул портала (#81.3, репо fleet), разделение PREV_CPU/PREV_NET sampler↔status (#81.4).
...
Последняя расходящаяся группа. Цветовые варианты кнопок вынесены в `/components.css` на `var()` (раньше index/network держали хардкод-hex → цветные кнопки **не адаптировались к тёмной теме**; services использовал `var()`). База `.btn-action` (геометрия) осталась на страницах — она реально разная (index/network — outline 40px, services — компактная solid; у services 7 plain-кнопок, свои `:hover`/`:disabled`, нет svg), сведение базы дало бы `min-height`-протечку и правку 18 JS-кнопок при нулевой визуальной пользе.

### Изменения вида (осознанные, подтверждены попиксельно)
- **Светлая тема — без изменений**: `var()` в light == прежние hex (проверено: c-green `#1D9E75`, c-amber `#BA7517`, … совпадают 1:1).
- **Тёмная тема — цветные кнопки index/network теперь адаптируются**: c-green `#1D9E75`→`#4ec894`, c-blue→`#6da8ff`, c-purple→`#a0a0ff`, c-red→`#ff6b6b` (был баг — застывший светлый цвет на тёмном фоне). c-amber остаётся `#BA7517`/`#EF9F27` (var(--warn) проваливает контраст на светлом).
- **services**: c-amber → контрастный `#BA7517` (был `var(--warn)`), c-purple/amber tint alpha 0.12→0.13/0.14, кнопки получили press-фидбек `:active` (как на дашборде).

`.chip` и `.dot` оставлены локальными (поведенческие/контекстные расхождения).
...
Продолжение 3.5.0. Общие компонентные классы вынесены в `components.css` (подключается после `tokens.css`). Внешний вид не изменился (проверено в браузере: computed-стили выносимых классов совпадают 1:1).

Выносить безопасно можно НЕ всё: страницы со временем разошлись. Анализатором проверено два условия — (1) класс определён идентично во всех файлах, (2) не используется на странице, где не стилится (иначе вынос «протёк» бы на неё). Вынесены только прошедшие оба:

### Added
- Новый публичный роут `/components.css` (в `_AUTH_PUBLIC_PATHS`). В install.sh + OTA.
- В `components.css` вынесены из index.html: `.tile-btn` (+ svg/:hover/:active/.primary/.success/.warn/.danger), `.range-toggle` (+button/.active), `.processes` (таблица), `.card-title/.card-value/.card-sub`, `.health-item` (+.icon/.label/.detail/статусы), `.ir-block`/`.ir-block-title`. Мобильные `@media`-оверрайды остались в index.html (ложатся поверх базы).
- `<link href=/components.css>` на всех 4 страницах (доступно всем; на network/services/login эти классы не используются — 0 влияния).

### Не вынесено (осознанно, остались локально)
- **Расходятся между страницами** (унификация = визуальная нормализация): `.btn-action` (services — компактный solid-вариант; network — без flex-direction/justify-content), `.card` (services-карточки кликабельны; login-карточка иная), `.header` (services — другой padding), `.container` (network — 1400px), `.pill`, `.chip`/`.chip.active`, `.dot`, `.modal-box`, `.header-actions`.
- **Используются без определения на др. странице** (вынос «протёк» бы): `.grid`, `.bar`, `.panel-title`, `.toast`, `.field-label`.

Эти классы — кандидаты на отдельный заход с нормализацией (при согласии на микро-правки вида).

## 3.5.0 — 2026-07-22

**Дизайн-система: централизация токенов в `/tokens.css`.**

CSS-переменные (`:root` + `@media(prefers-color-scheme)` + `html[data-theme]`) были **продублированы 4 раза в каждом** из index/network/services/login. Вынесены в единый `tokens.css` — один источник истины. Внешний вид не изменился (значения выверены 1:1 с живым кодом).

### Changed
- Новый статик-роут `/tokens.css` (публичный, в `_AUTH_PUBLIC_PATHS` — login отдаётся неаутентифицированным). Добавлен в install.sh и OTA (ext `.css` уже копировался апдейтером).
- В `<head>` каждой страницы `<link rel="stylesheet" href="/tokens.css">` перед инлайн-`<style>`; дублирующие блоки токенов удалены. Компонентные классы и страничные стили не тронуты.

### Fixed (побочно, при сверке)
- `services.html` не имел `html[data-theme]`-блоков → ручной тумблер темы там не работал (только системная `prefers-color-scheme`). Теперь работает как на других страницах.
- Прочие мелкие расхождения значений между страницами устранены сведением к одному набору (dark `--purple` = `#a0a0ff` во всех).

### Примечание по хендоффу
Пакет «SmartComm Design System» применён выборочно: токены — да; черновой `components.css` НЕ использован (в нём 2 из 3 `[VERIFY]`-правил и dark `--purple` расходились с живым кодом). Централизация компонентных классов — отдельным выверенным заходом при необходимости.

## 3.4.5 — 2026-07-22

**SSE live-tail логов: убрана утечка потоков waitress (HIGH #80).**

`stream_logs` читал вывод `docker compose logs --follow` через блокирующий `proc.stdout.readline()`. На **тихом** контейнере (нет новых строк) readline блокировал поток навсегда: keepalive не отправлялся и — главное — **отвал клиента не детектился**, поэтому waitress-поток и docker-follow-процесс висели вечно. При лимите `_SSE_MAX=4` после 4 таких утечек live-tail отдавал 503 и переставал работать до рестарта.

### Fixed
- Неблокирующее чтение через `select` + сырой `os.read` по fileno (`bufsize=0`, режем по `\n` сами). Цикл просыпается каждые 5с даже в тишине: раз в 15с шлёт keepalive — он же ловит отвал клиента (`yield` на закрытый сокет → `GeneratorExit` → `finally` завершает docker-процесс и освобождает поток). Теперь мёртвый стрим прибирается ≤15с вместо «никогда».
- Соседний `_stream_subprocess` (install/uninstall progress) не затронут — он в фоновом воркере с `threading.Timer`-таймаутом (блокировка ограничена, поток waitress не держит).

## 3.4.4 — 2026-07-22

**Само-обновление: health-check + авто-rollback + атомарность + disk-check (HIGH из аудита).**

Раньше `_apply_update` качал tarball, делал бэкап и **безусловно** рестартовал сервис (`sleep 2 && systemctl restart`) — без проверки места, без валидации релиза и **без проверки, что новая версия вообще поднялась**. Битый релиз (syntax error / падение на импорте) укладывал дашборд контроллера намертво, отката не было. Теперь полный цикл с самовосстановлением:

### Added / reliability
- **Disk-check** перед скачиванием: если свободного места меньше, чем нужно на бэкап+tarball+распаковку — апдейт не стартует (частый кейс забитой SD на Pi).
- **Атомарность:**
  - pre-validate: все новые `.py` компилируются (`py_compile`) **до** касания живого каталога — битый релиз не применяется даже частично;
  - per-file `os.replace` через `<name>.new` — обрыв в середине не оставит полуфайла.
- **Health-check + авто-rollback** через `_launch_update_verifier`: рестарт, health-check и откат выполняет отдельный **transient systemd-unit** (`systemd-run`, своя cgroup — переживает рестарт дашборда, обычный child был бы убит `KillMode=control-group`). Верификатор рестартит сервис, до 60с ждёт HTTP 200 на публичном `/login` (доказывает, что новый код реально загрузился), и при отсутствии здоровья **восстанавливает бэкап и рестартит обратно** на прошлую версию. Если `systemd-run` недоступен — fallback на обычный отложенный рестарт (апдейт применяется без авто-проверки).
- Исход верификации виден оператору: `/api/update/state` отдаёт `verify_log` (хвост лога `SUCCESS`/`ROLLED_BACK`) — in-memory `last_apply` теряется при рестарте, поэтому читаем с диска.
- Оба пути апдейта (ручной `/api/update/apply` и команда `update` от fleet-портала) идут через новый безопасный механизм.
